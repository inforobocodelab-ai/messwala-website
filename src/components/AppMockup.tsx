...
import { TIFFIN_PROVIDERS, HOSTEL_LISTINGS } from '../data/mockData';
...
export const AppMockup: React.FC<AppMockupProps> = ({ selectedCity, onBookTrial }) => {
  const [activeTab, setActiveTab] = useState<'tiffin' | 'hostel'>('tiffin');
  const [searchQuery, setSearchQuery] = useState('');
  const [favorites, setFavorites] = useState<string[]>(['tiffin-1']);
  const [selectedMealType, setSelectedMealType] = useState<'all' | 'veg' | 'non-veg'>('all');
  ...
  const filteredTiffins = TIFFIN_PROVIDERS.filter(t => {
    ...
    const matchType = selectedMealType === 'all' ? true : selectedMealType === 'veg' ? t.type === 'veg' : t.type === 'non-veg' || t.type === 'both';
      ...
      <div className="w-full h-full bg-[#FAF8F5] rounded-[38px] overflow-hidden flex flex-col relative pt-7">
        ...
        <div className="px-4 pt-3 pb-2 bg-white">
          <div className="relative flex items-center">
            ...
            />
            <span className="absolute right-3 w-4 h-4 text-[10px] font-bold text-slate-400 bg-slate-200 rounded-full flex items-center justify-center">
              0
            </span>
          </div>

          {/* Tab Switcher: Tiffin / Hostel */}
          <div className="flex items-center space-x-6 pt-3 border-b border-slate-100">
            <button
              onClick={() => setActiveTab('tiffin')}
              className={`pb-2 text-xs font-bold transition-all relative ${
                activeTab === 'tiffin' ? 'text-slate-900' : 'text-slate-400'
              }`}
            >
              Tiffin
              {activeTab === 'tiffin' && (
            ...
            <button
              ...
              className={`pb-2 text-xs font-bold transition-all relative ${
                activeTab === 'hostel' ? 'text-slate-900' : 'text-slate-400'
              }`}
            >
              Hostel
              {activeTab === 'hostel' && (
        ...
        <div className="flex-1 overflow-y-auto px-3 py-3 space-y-3 scrollbar-thin">
          ...
          {activeTab === 'tiffin' ? (
            <>
                ...
                >
                  <div className="flex items-center justify-between mb-2">
                    ...
                    >
                      <Heart
                        className={`w-4 h-4 ${
                          favorites.includes(tiffin.id) ? 'fill-red-500 text-red-500' : ''
                        }`}
                      />
                    </button>
                  </div>
                  ...
                  <div className="flex items-start justify-between">
                    <div>
                      <h4 className="text-xs font-extrabold text-slate-900 line-clamp-1 group-hover:text-red-600 transition-colors">
                        {tiffin.name}
                      </h4>
                      <div className="flex items-center space-x-1 text-[10px] text-slate-500 mt-0.5">
                        <span
                          className={`w-2 h-2 rounded-xs inline-block ${
                            tiffin.type === 'veg' ? 'bg-emerald-500' : 'bg-red-500'
                          }`}
                        ></span>
              ...
              {/* Action Banner in Mockup */}
              <div className="p-3 bg-gradient-to-r from-red-500 to-orange-500 text-white rounded-2xl text-center space-y-1">
                <p className="text-[10px] font-semibold text-orange-100">Special Student Discount</p>
                <p className="text-xs font-black">Get ₹200 OFF on 1st Month Subscription</p>
                <p className="text-[9px] bg-white/20 py-0.5 px-2 rounded-full inline-block font-mono">
                  CODE: MESSWALA50
                </p>
              </div>
            </>
            ...
            /* Hostel Listings Tab */
            <>
              {HOSTEL_LISTINGS.map((hostel) => (
                <div
                  key={hostel.id}
                  className="bg-white rounded-2xl p-3 border border-orange-100 shadow-xs hover:shadow-md transition-all cursor-pointer"
                >
                  <div className="relative h-24 rounded-xl overflow-hidden mb-2">
                    ...
                    />
                    <span className="absolute top-2 left-2 px-2 py-0.5 bg-slate-900/80 text-white text-[9px] font-bold rounded-md backdrop-blur-xs">
                      {hostel.type} PG
                    </span>
                    <span className="absolute bottom-2 right-2 px-2 py-0.5 bg-emerald-600 text-white text-[9px] font-bold rounded-md">
                      Messwala Included
                    </span>
                  </div>

                  <h4 className="text-xs font-extrabold text-slate-900">{hostel.name}</h4>
                  <p className="text-[10px] text-slate-500 mt-0.5">{hostel.distanceToMajorCollege}</p>

                  <div className="mt-2 flex items-center justify-between text-xs">
                    <span className="font-black text-slate-900">₹{hostel.monthlyRent}/mo</span>
        ...
        {/* Floating Book Button matching uploaded image */}
